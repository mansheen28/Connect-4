while (!player1){
    var player1 = prompt('Player One: Enter your name:');   //dialog box
};
let player1Color = '#263238';

while (!player2){
    var player2 = prompt('Player Two: Enter your name');
};
let player2Color = '#4db6ac';
...
let circleSlot = document.querySelectorAll('.circle');
let resetBtn = document.querySelector('.reset-bar');
...
for (i = 0; i < tableData.length; i ++){
    tableData[i].addEventListener('click', function(e){
        console.log(`${e.target.parentElement.rowIndex},${e.target.cellIndex}`)   //row and column 
    });
};
...
function changeColor(e){
    ...
    for (i = 5; i > -1; i--){
        if (tableRow[i].children[column].style.backgroundColor == 'white'){
            ...
            if (currentPlayer === 1){
                row[0].style.backgroundColor = '#263238';
                ...
                row[0].style.backgroundColor = '#4db6ac';
                if (horizontalCheck() || verticalCheck() || diagonalCheck() || diagonalCheck2()){
...
Array.prototype.forEach.call(tableData, function(cell) {
